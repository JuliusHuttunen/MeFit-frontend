...
const Goal = (props) => {
  ...
  const markExerciseAsCompleted = async (goal, boolean, exerciseId) => {
    const [error, response] = await setExerciseCompleted(
      goal,
      boolean,
      exerciseId
    );
  ...
  const markWorkoutAsCompleted = async (goal, boolean, workoutId) => {
    const [error, response] = await setWorkoutCompleted(
      goal,
      boolean,
      workoutId
    );
  ...
  return (
    <Container>
      <Accordion key={props.index}>
        <Accordion.Item
          style={{
            backgroundColor: props.achieved
              ? "#defade"
              : props.history
              ? "#F2D4D7"
              : "white",
        ...
        >
          <Accordion.Header>
            {props.history ? (
              <h4 style={{ marginRight: "2em" }}>
                Goal ({format(new Date(props.goal.startDate), "dd.MM.yyyy")}-
                {format(new Date(props.goal.endDate), "dd.MM.yyyy")})
              </h4>
            ) : (
              <>
                <h4 style={{ marginRight: "2em" }}>Goal #{props.counter}</h4>
                <h4 style={{ fontStyle: "italic" }}>
                  {props.difference === 0
                    ? "The goal should be completed today!"
                    : props.difference + " whole day(s) left"}
                </h4>
              </>
            )}
          </Accordion.Header>
          <Accordion.Body>
            <div>
              <h4>
                Goal time period:{" "}
                {format(new Date(props.goal.startDate), "dd.MM.yyyy")}-
                {format(new Date(props.goal.endDate), "dd.MM.yyyy")}
              </h4>
            </div>
            {currentProgram !== null ? (
              <>
                <Program program={currentProgram} index={1}></Program>
                <hr />
              </>
            ) : (
              <></>
            )}
            {props.goal.workouts.length !== 0 ? (
              <>
                <Accordion key={3}>
                  <Accordion.Item key={3} eventKey={3}>
                    ...
                    <Accordion.Body>{workoutMap}</Accordion.Body>
                  </Accordion.Item>
                </Accordion>
                <hr />
              </>
              ...
              <>
                <Accordion key={2}>
                  <Accordion.Item key={2} eventKey={2}>
                    ...
                    <Accordion.Body>{exerciseMap}</Accordion.Body>
                  </Accordion.Item>
                </Accordion>
                <hr />
              </>
            ...
            <Container>
              <Row style={{ width: "50%" }}>
                {props.goal.achieved ? (
                  <Button
                    variant="secondary"
                    onClick={() => markGoalAsCompleted(props.goal, false)}
                  >
                    Revert
                  </Button>
                ) : (
                  <Button
                    variant="success"
                    onClick={() => markGoalAsCompleted(props.goal, true)}
                  >
